...
class QuotaStore:
    def __init__(self, prefix: str = "freellm") -> None:
        self._prefix = prefix
    ...
    async def get(self, key: str) -> int:
        return await self._memory.get(self._full_key(key))

    async def incr(self, key: str, amount: int = 1, ttl: int | None = None) -> int:
        return await self._memory.incr(self._full_key(key), amount=amount, ttl=ttl)
